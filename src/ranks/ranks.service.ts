...
export class RanksService {
  ...
  async getAll(currentRegion) {
    ...
    try {
      ...
      const regionForCount = await this.regions.findOne({
        where: { regionSi, regionGu, regionDong },
      });
      const { totalCount } = regionForCount; // 현재 지역 전체 주소
      ...
      // 현재 지역과 일치하는 모든 날짜의 지역 조회 (완료 테이블, 완료한 플레이어 조인)
      const quests = await Promise.all([
        ...regions.map(async (region) => {
          return await this.quests.find({
            where: { region },
            // TODO: relations 내에서 필요한 정보만 받을 수 있게 수정
  ...
  async rankFor(
    ...
    console.log(countFor); // { '96': 2, '99': 1, '100': 4 }
